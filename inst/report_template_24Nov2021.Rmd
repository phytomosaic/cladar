...
- \usepackage{caption}
params:
  unit: 'Bridger-Teton National Forest'  # <--- hard-coded for TESTING ONLY
output:
  pdf_document:
    toc: yes
    number_sections: yes
...
require(kableExtra)
`one` <- function(x) formatC(round(as.numeric(x),1),format='f',digits=1)
```
...
# prefixes (grab from source dir)
admin_id <- 'USFS_60'         # hard-coded for TESTING ONLY, USFS_60 = BTNF
prefix   <- paste0('../data/', admin_id, '_table_')

# filenames vector; only 5 thru 17 are used
fnm <- paste0(prefix, 1:17, '.csv')

# read CSV files (if they exist), then assign to objects f1,f2,f3,etc
sapply(seq_along(fnm), 
       function(i) {
            if (file.exists(fnm[i])) {
                 assign(paste0('f',i), read.csv(fnm[i], strip.white=TRUE), envir=.GlobalEnv)
            } else {
                 cat(fnm[i], 'file not found\n')
            }
         return(NA)
       }
)
...
| Herb minimum: | `r if(exists('f12')) { one(min(f12$min_cl[f12$min_cl > 0], na.rm=T)) } else {''}`  | `r if(exists('f13')) { one(min(f13$min_cl[f13$min_cl > 0], na.rm=T)) } else {''}`  |
| Herb median:  | `r if(exists('f12')) { one(median(c(f12$min_cl, f12$max_cl)[c(f12$min_cl, f12$max_cl)>0], na.rm=T)) } else {''}`  | `r if(exists('f13')) { one(median(c(f13$min_cl, f13$max_cl)[c(f13$min_cl, f13$max_cl)>0], na.rm=T)) } else {''}`  |
| Herb maximum: | `r if(exists('f12')) { one(max(f12$max_cl[f12$max_cl > 0], na.rm=T)) } else {''}`  | `r if(exists('f13')) { one(max(f13$max_cl[f13$max_cl > 0], na.rm=T)) } else {''}`  |
...
| Lichen minimum: | `r '1.3'`  | `r '2.3'`  |
| Lichen maximum: | `r '3.5'`  | `r '6.0'`  |
...
| -------- | -------- | -------- | ------: | ------: |
| Deposition | Model | Deposition minimum    | `r one(f4$n_min)`  |  `r one(f4$s_min)`  |
| Deposition | Model | Deposition maximum    | `r one(f4$n_max)`  |  `r one(f4$s_max)`  |
...
| Aquatic |  Plot | Aquatic N- to P-limitation, fixed | `r '4.1'` |  --  |       <!-- NEWLY ADDED -->
| Herb    |  Plot |             Herb richness (open, plot min.) |  `r if(exists('f8')) { one(f8$min[f8$ecosystem == 'open']) } else {'--'}`  |  --  |  <!-- f8 = simkin -->
| Herb    | Model |          Herb richness (open, surface min.) |  `r if(exists('f10')) { one(f10$min[f10$ecosystem == 'open']) } else {'--'}`  |  --  |  <!-- f10 = surface -->
| Herb    | Model |  Herb richness (open, surface 10\% decline) |  `r if(exists('f10')) { one(f10$min_10[f10$ecosystem == 'open']) } else {'--'}`  |  --  |  <!-- f10 = surface -->
| Herb    |  Plot |           Herb richness (closed, plot min.) |  `r if(exists('f8')) { one(f8$min[f8$ecosystem == 'closed']) } else {'--'}`  |  --  |  <!-- f8 = simkin -->
| Herb    | Model |        Herb richness (closed, surface min.) |  `r if(exists('f10')) { one(f10$min[f10$ecosystem == 'closed']) } else {'--'}`  |  --  |  <!-- f10 = surface -->
| Herb    | Model | Herb richness (closed, surface 10\% decline)|  `r if(exists('f10')) { one(f10$min_10[f10$ecosystem == 'closed']) } else {'--'}`  |  --  |  <!-- f10 = surface -->
| Herb    |  Area |                          Pardo herb minimum |  --  |  --  |  <!-- no data? -->
| Herb    |  Area |                          Pardo herb maximum |  --  |  --  |  <!-- no data? -->
| Herb    |  Plot |  Herb occurrence minimum | `r if(exists('f12')) { one(min(f12$min_cl[f12$min_cl > 0], na.rm=T)) } else {''}`  | `r if(exists('f13')) { one(min(f13$min_cl[f13$min_cl > 0], na.rm=T)) } else {''}`  |  <!-- f12 = clark-n / f13 = clark-s -->
| Herb    |  Plot |  Herb occurrence median  | `r if(exists('f12')) { one(median(c(f12$min_cl, f12$max_cl)[c(f12$min_cl, f12$max_cl)>0], na.rm=T)) } else {''}`  | `r if(exists('f13')) { one(median(c(f13$min_cl, f13$max_cl)[c(f13$min_cl, f13$max_cl)>0], na.rm=T)) } else {''}`  |  <!-- f12 = clark-n / f13 = clark-s -->
| Herb    |  Plot |  Herb occurrence maximum | `r if(exists('f12')) { one(max(f12$max_cl[f12$max_cl > 0], na.rm=T)) } else {''}`  | `r if(exists('f13')) { one(max(f13$max_cl[f13$max_cl > 0], na.rm=T)) } else {''}`  |  <!-- f12 = clark-n / f13 = clark-s -->
...
| Lichen | Model | Lichen minimum | `r '1.3'`  | `r '2.3'`  |
| Lichen | Model | Lichen maximum | `r '3.5'`  | `r '6.0'`  |
| Lichen | Model | Lichen total richness             | `r '3.5'`  | `r '6.0'`  |   
| Lichen | Model | Lichen total richness 30\% decline | `r '5.5'`  | `r '10.1'`  |
| Lichen | Model | Sensitive lichen richness | `r '3.1'`  | `r '2.5'`  |
| Lichen | Model | Sensitive lichen richness 30\% decline | `r '4.7'`  | `r '3.6'`  |
| Lichen | Model | Forage lichen abundance | `r '1.9'`  | `r '2.6'`  |
| Lichen | Model | Forage lichen abundance 30\% decline | `r '2.8'`  | `r '3.8'`  |
| Lichen | Model | Cyanolichen abundance | `r '1.3'`  | `r '2.3'`  |
| Lichen | Model | Cyanolichen abundance 30\% decline | `r '1.9'`  | `r '3.2'`  |
| Lichen | Model | Lichen community airscores | `r '1.5'`  | `r '2.7'`  |
...
<!-- | Herb | Plot | Herb richness 10\% decline |  --  |  --  |     # f12 = clark-n / f13 = clark-s -->
...
```{r table_06_aquatic_shifts, message=FALSE, warning=FALSE}
...
```{r table_12_herb_species_nitrogen}
if (exists('f12')) {
  ...
  cnames <- c('Species', 'Response', 'Count', 'Max CL', 'Min CL', 'Min dep', 'Max dep', 'Points < CL', 'Points at CL', 'Points > CL')
...
knitr::kable(tab_12, 
    ...
    column_spec(1, width='2.1in', italic=TRUE) %>% 
    column_spec(2, width='0.55in') %>% 
    column_spec(3, width='0.20in') %>% 
  column_spec(4:7, width='0.30in') %>% 
  column_spec(8:10, width='0.35in') %>%
  kable_styling(latex_options = 'repeat_header')
```

\newpage  

```{r table_13_herb_species_sulfur}
if (exists('f13')) {
  ...
  cnames <- c('Species', 'Shape', 'Count', 'Max CL', 'Min CL', 'Min dep', 'Max dep', 'Points < CL', 'Points at CL', 'Points > CL')
...
```{r table_17_lichen, message=FALSE, warning=FALSE}
if (exists('f17')) {
